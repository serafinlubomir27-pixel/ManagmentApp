...
def get_all_users(current_user: dict = Depends(require_admin)):
    """Všetci používatelia VLASTNEJ organizácie. Len pre admin."""
    return user_repo.get_all_users(current_org_id(current_user))


@router.get("/tree")
def get_hierarchy_tree(current_user: dict = Depends(require_manager_or_admin)):
    """Celý strom hierarchie pod aktuálnym manažérom/adminom (BFS), v rámci jeho organizácie."""
    return get_full_tree(current_user["id"], current_org_id(current_user))
...
def update_user(
    ...
    """Aktualizovať rolu alebo manažéra používateľa. Len admin VLASTNEJ organizácie."""
    # assert_can_view_user overí existenciu aj zhodu organizácie — bez toho by admin
    # organizácie A vedel meniť rolu používateľovi organizácie B.
    assert_can_view_user(user_id, current_user)
    if body.role:
